...
for element in target_list:                 # Цикл для удаления обрамляющих кавычек
    if element == '""':
        target_list.remove(element)
result_str = ' '.join(target_list)

print(some_list)                            # Исходный лист для сравнения
print(result_list)                          # Новый лист в котором число обособлено кавычками
print(result_str)                           # Срока в требуем формате
